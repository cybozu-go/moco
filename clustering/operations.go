...
func (p *managerProcess) configureReplica(ctx context.Context, ss *StatusSet, index int) (redo bool, e error) {
	...
	// if the binlog required by the replica instance is not existing in the new primary instance, some data may be missing when `CHANGE REPLICATION SOURCE TO' is executed.
