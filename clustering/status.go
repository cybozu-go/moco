...
func isDegraded(ss *StatusSet) bool {
	...
	if replicasInCluster(ss.Cluster, pst.ReplicaHosts) < int32(RequiredSemiSyncAcks(int(ss.Cluster.Spec.Replicas))) {
		return false
	}
	...
	return okReplicas >= RequiredSemiSyncAcks(int(ss.Cluster.Spec.Replicas)) && okReplicas != int(ss.Cluster.Spec.Replicas-1)
}
...
func isFailed(ss *StatusSet) bool {
	...
	return okReplicas > RequiredSemiSyncAcks(int(ss.Cluster.Spec.Replicas))
}
