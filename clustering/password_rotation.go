...
// current rotation Phase. It returns (true, nil) when progress was made and
...
func (p *managerProcess) handlePasswordRotation(ctx context.Context, ss *StatusSet) (bool, error) {
	phase := ss.Cluster.Status.SystemUserRotation.Phase

	switch phase {
	case mocov1beta2.RotationPhaseRotating:
		return p.handleRotatingPhase(ctx, ss)
	case mocov1beta2.RotationPhaseDiscarding:
		return p.handleDiscardingPhase(ctx, ss)
	default:
		return false, nil
	}
}
...
func (p *managerProcess) handleRotatingPhase(ctx context.Context, ss *StatusSet) (bool, error) {
	...
	if !hasPending {
		log.Info("waiting for controller to generate pending passwords", "rotationID", rotation.RotationID)
		return false, nil
	}

	replicas := int(cluster.Spec.Replicas)
	if replicas == 0 {
		log.Info("waiting for replicas to be scaled up before RETAIN", "rotationID", rotation.RotationID)
		return false, nil
	}
...
func (p *managerProcess) handleDiscardingPhase(ctx context.Context, ss *StatusSet) (bool, error) {
	log := logFromContext(ctx)
	cluster := ss.Cluster
	rotation := &cluster.Status.SystemUserRotation
	...
	if !hasPending {
		log.Info("waiting for pending passwords for discard", "rotationID", rotation.RotationID)
		return false, nil
	}

	replicas := int(cluster.Spec.Replicas)
	if replicas == 0 {
		log.Info("waiting for replicas to be scaled up before DISCARD", "rotationID", rotation.RotationID)
		return false, nil
	}
