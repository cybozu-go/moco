...
func (o *operator) GetUserAuthPlugin(ctx context.Context, user string) (string, error) {
	if err := validateMocoUser(user); err != nil {
		return "", err
	}
	var plugin string
	query := "SELECT plugin FROM mysql.user WHERE User = ? AND Host = '%'"
	if err := o.db.GetContext(ctx, &plugin, query, user); err != nil {
		return "", fmt.Errorf("failed to get auth plugin for %s: %w", user, err)
	}
	return plugin, nil
}
