...
| observedRotationGeneration | ObservedRotationGeneration is the last rotationGeneration whose rotation phase (RETAIN + pending-password distribution) completed successfully. Equality with spec.rotationGeneration is a necessary condition for the cycle to leave the rotation phase, but not sufficient on its own: RotationReady=True is only set at the very end of the full cycle (after the discard phase finalises). | int64 | true |
