...
| slowQueryLogConfigTmpl | SlowQueryLogConfigTmpl is the template for slow query log configuration. The default is to use the default slow query log configuration. `{{ .Path }}` will be replaced with the path to the slow query log file. | *string | false |
